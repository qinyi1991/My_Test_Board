...
功能函数：
         1、DataScope_Get_Channel_Data(float Data,CPU_INT08U Channel) ：
				    此函数无返回。
         ...
         2、DataScope_Data_Generate(CPU_INT08U Channel_Number) ：
				    返回一个单字节无符号整型。
...
//MDK下库文件使用例子（stm32 发送10个通道数据）：
   ...
   {
	   CPU_INT08U i;          //计数变量
		 CPU_INT08U Send_Count; //串口需要发送的数据个数
...
extern CPU_INT08U DataScope_OutPut_Buffer[42];	   //待发送帧数据缓存区
...
void DataScope_Get_Channel_Data(CPU_FP32 Data,CPU_INT08U Channel);    // 写通道数据至 待发送帧数据缓存区

CPU_INT08U DataScope_Data_Generate(CPU_INT08U Channel_Number);        // 发送帧数据生成函数 
